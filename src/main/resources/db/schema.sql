...
drop view if exists screening_places;
...
SELECT DISTINCT p.id,
                ...
                CASE
                    WHEN t.status = 'ENABLE' THEN 'BOUGHT'
                    WHEN r.status = 'ENABLE' and r.expiry_date > current_timestamp THEN 'RESERVED'
                    WHEN p.unused = True THEN 'UNUSED'
                    ELSE 'FREE'
                    END :: varchar(8) as status_seat
FROM places as p
         JOIN halls h on h.id = p.hall_id
         JOIN film_screenings fs on p.hall_id = fs.hall_id
         LEFT OUTER JOIN tickets t on p.id = t.place_id
         LEFT OUTER JOIN reservations r on p.id = r.place_id
